...
def compute_distances(Xtrain, X):
	...
	for i in range(X.shape[0]):
		dists[i] = np.sqrt(np.sum(np.square(X[i] - Xtrain), axis=1))
	#####################################################		 
	return dists
...
def predict_labels(k, ytrain, dists):
	...
	K_n_n = ytrain[sorted_index]
	for data_point in K_n_n:
		votes = np.bincount(data_point)
		ypred = np.append(ypred, np.argmax(votes))
	#####################################################
	return ypred
...
def find_best_k(K, ytrain, dists, yval):
	...
	min_acc_index = validation_accuracy.index(max(validation_accuracy))
	best_k = K[min_acc_index]
	#####################################################
