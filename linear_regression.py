...
def linear_regression_noreg(X, y):
  ...
  X = np.array(X)
  y = np.array(y)
  cov_mat = np.linalg.inv(np.matmul(np.transpose(X), X))
  w = np.array(np.matmul(np.matmul(cov_mat, np.transpose(X)), y))
  #####################################################		 
  return w
...
def regularized_linear_regression(X, y, lambd):
  ...
  w = np.matmul(np.matmul(np.linalg.inv(cov_mat + lambd * np.identity(np.size(X,1))), np.transpose(X)), y)
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  mse_s = []
  for lambd in lambds:
    w = np.linalg.inv(np.dot(Xtrain.transpose(),Xtrain) + lambd*np.identity((Xtrain.shape)[1])).dot(Xtrain.transpose()).dot(ytrain)
    mse = np.mean((yval-np.dot(w, Xval.transpose()))**2)
    mse_s.append(mse)

  min_mse_index = mse_s.index(min(mse_s))
  bestlambda = lambds[min_mse_index] 
  #####################################################		 
  return bestlambda
...
def test_error(w, X, y):
  """
    ...
    """
  err = np.mean((y-np.dot(w, X.transpose()))**2)
  return err
