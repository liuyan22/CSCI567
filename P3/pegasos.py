...
def objective_function(X, y, w, lamb):
    ...
    N = X.shape[0]
    obj_value = obj_value = lamb/2.0*(np.sum(w**2)) + 1.0/N*np.sum(np.maximum(np.zeros((1,N)),np.ones((1,N))-np.multiply(np.expand_dims(y,axis=0),np.dot(w.transpose(), X.transpose()))))
    return obj_value
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        y_t_plus = []
        for i in A_t:
            if ytrain[i]*np.dot(np.transpose(w), Xtrain[i]) < 1:
                A_t_plus.append(Xtrain[i])
                y_t_plus.append(ytrain[i])
        A_t_plus = np.array(A_t_plus)
        y_t_plus = np.array(y_t_plus)
        num = y_t_plus.shape[0]
        stepSize = 1.0/(lamb*iter)
        if num != 0:
            w_half = (1.0-stepSize*lamb)*w + stepSize/k*np.expand_dims((np.sum(np.multiply(np.repeat(np.expand_dims(y_t_plus,axis=0), D, axis=0).transpose(), A_t_plus),axis=0)),axis=0).transpose()
        else:
            w_half = (1.0-stepSize*lamb)*w
        w = min(1, (1/np.sqrt(lamb))/np.sqrt(np.sum(w_half**2))) * w_half
...
def pegasos_test(Xtest, ytest, w_l):
    ...
    test_acc = np.mean((2*(np.dot(w_l.transpose(), Xtest.transpose()) > t)-1) == ytest)
